...
# ===== Kirim Email =====
def send_encryption_email(receiver_email, encrypted_data, key_hex, filename):
    msg = MIMEMultipart()
    ...
    body = f"""
Dokumen berhasil dienkripsi dan dilampirkan sebagai PDF terenkripsi.
...
"""
    ...
    attachment.add_header("Content-Disposition", "attachment", filename=f"encrypted_{filename}.pdf")
...
if menu == "Enkripsi":
    ...
    uploaded_file = st.file_uploader("📄 Unggah file (.txt / .pdf)", type=["txt", "pdf"])
        ...
        if not uploaded_file or not email_receiver:
            ...
            if not re.match(r"[^@]+@[^@]+\.[^@]+", email_receiver):
                ...
                try:
                    ...
                    send_encryption_email(email_receiver, encrypted_data, key.hex(), filename)
                    ...
                    st.download_button(
                        label="⬇️ Unduh File Terenkripsi (.pdf)",
                        data=encrypted_data,
                        file_name=f"encrypted_{filename}.pdf",
...
# ========================================
    ...
    st.header("🔓 Dekripsi File Terenkripsi (.pdf)")
    encrypted_file = st.file_uploader("📤 Unggah file terenkripsi (.pdf)", type=["pdf"], key="decrypt_file")
    key_hex_input = st.text_input("🔑 Masukkan Kunci 3DES (hex)", key="decrypt_key")
    output_filename = st.text_input("📁 Nama file hasil dekripsi (cth: hasil.txt atau hasil.pdf)", key="decrypt_name")
